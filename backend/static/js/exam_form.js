        ...
        element.innerHTML = `
                    <div class="card-header d-flex justify-content-between py-3">
                        ...
                        <div class="d-flex justify-content-around" style="border-style: none;padding: 0px;">
                            <p class="text-primary m-0 fw-bold">Score</p><input type="text" class="form-control positive" data-qid="${result["question"].id}" style="height: 22px;width: 55px;margin: 0px;margin-right: 8px; margin-left: 8px;" placeholder="+Ve" />
                        </div>
                    </div>
                        <div class="card-body">
                                <div class="row">
                                    ...
                                    <div class="col">
                                        <div class="mb-3 choices" data-id="${result["question"].id}">
                                            ...
                                            <div class="form-check justify-content-between d-flex" data-id="${result["question"].id}">
                                                <input class="form-check-input" id="${result["choices"].id}" name="${result["question"].id}" data-qid="${result["question"].id}" disabled type="radio" >
                                ...
                                <div class="row">
                                    <div class="col">
                                        <div class="mb-3 choices" data-id="${result["question"].id}">
                                            <label class="form-label" for=""><strong>Options</strong><br></label>
                                            <div class="form-check justify-content-between d-flex" data-id="${result["question"].id}">
                                                <input class="form-check-input" id="${result["choices"].id}" data-qid="${result["question"].id}" disabled type="checkbox" >
                                                <input class="form-control mx-2 edit-choice" value="${result["choices"].optn}" data-id="${result["choices"].id}">
                                                <span class="remove-option" title = "Remove" data-id="${result["choices"].id}"><i class="fa-solid fa-xmark"></i></span>
                                            </div>
                                            <button class="btn m-1 btn-outline-dark btn-sm add-option" type="button" data-question="${result["question"].id}" data-type="${result["question"].q_type}" style="width: 30px;height: 31px;padding: 6px 12px;padding-left: 10px;padding-right: 10px;padding-top: 4px;padding-bottom: 4px;"><Strong>+</Strong></button></div>
...
//  // <=========================================================================================================================================================================>
            ...
            question.addEventListener("click", function(){
                ...
                .then(result => {
                    ...
                    if(this.dataset.type === "mcq"){
                        element.innerHTML = `
                        <input class="form-check-input flex-fill" id="${result["id"]}" name="${question_id}" data-qid="${question.id}" disabled type="radio" >
                        ...
                        element.innerHTML = `
                        <input class="form-check-input flex-fill" id="${result["id"]}" name="${question_id}" data-qid="${question.id}" disabled type="checkbox" >
                        <input class="form-control mx-2 edit-choice" value="${result["choice"]}" data-id="${result["id"]}">
                        <span class="remove-option" title = "Remove" data-id="${result["id"]}"><i class="fa-solid fa-xmark"></i></span>
                        `;
                    }
    ...
    const addScore = () => {
        document.querySelectorAll(".positive").forEach(option =>{
            option.addEventListener("input", async function(){
                await fetch(`add_score/${this.dataset.qid}`,{
                    method: "POST",
                    headers: {'X-CSRFToken': csrf},
                    body: JSON.stringify({
                        "id": this.dataset.qid,
